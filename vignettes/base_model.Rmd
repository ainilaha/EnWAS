...
### 3.1 Check socioeconomic status (SES) impacts

The ratio of family income to poverty (`INDFMPIR`) has reasonable distribution in the range of 0 to 4.99 and has a peak at 5 because 5 represents the values greater than or equal to 5.00. There is no clear pattern in the smooth scatter plot of diastolic and INDFMPIR.

```{r base_model}
layout(matrix(c(1,2), ncol=2, byrow=TRUE))
par(mar=c(4.0,4.0,3.5,1))
hist(base_df$INDFMPIR)
smoothScatter(base_df$INDFMPIR,base_df$DIASTOLIC) 
```

Now, we can check the impacts of SES with ANOVA LRT. Sum of Sq = 1991.423/3741194=0.0005322961=0.05%, which indicates contribution is too small. In addition, we may have to remove some the data record where INDFMPIR values are NA. Therefore, INDFMPIR may not worth to include base model.
...
ns_str_in <-
  'DIASTOLIC ~ ns(RIDAGEYR, knots = seq(30, 80, by = 10), Boundary.knots=c(20,90)) * RIAGENDR + ns(BMXBMI,knots = c(seq(15, 45, by = 5),seq(45,65,by=10)),Boundary.knots=c(10,85)) + RIDRETH1+ns(INDFMPIR,knots = seq(0, 5, by = 0.5),Boundary.knots=c(-0.5,5.5))'
ns_base_in <- lm(formula = as.formula(ns_str_in), base_df[!is.na(base_df$INDFMPIR),])
print_anova(anova(ns_base,ns_base_in,test="LRT"))
```

Similarly, we can find the Sum of Sq for `DMDEDUC2` is 2865.658/4091156=0.0007004519=0.07%, which also considerably small.
