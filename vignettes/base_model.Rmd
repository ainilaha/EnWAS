...
* we may need to wrap up as function and slightly change the the process with PHESANT-like page.
...
lm_base <- glm(formula = as.formula(lm_str), family = gaussian,base_df)
...
ns_base <- glm(formula = as.formula(ns_str), family = gaussian,base_df)
```
...
base_raw_g <- ggplot(mpred_df,
  ...
  xlab("Age (year)")+ylab("Diastolic (mm Hg)")+ facet_grid(cols = vars(variable))+
...
We can drop of the terms in model to produce sub models and compare with full model, and the results can be produce as follow. *ratio=Deviance/Resid. Dev*

```{r anova2, echo=TRUE,results = "asis"}
lm_anvoa <- anova_lrt(lm_str,data = base_df)
knitr::kable(lm_anvoa)
ns_anvoa <- anova_lrt(ns_str,data = base_df)
